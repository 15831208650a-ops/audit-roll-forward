...
def copy_q1_review_images(prior_path, output_path, q105_row_shifts=None):
    ...
    for sheet_prefix, minimum_anchor_row in selectors.items():
        ...
        if any(rel.attrib.get("Type") == drawing_rel_type for rel in output_sheet_rels):
            continue
        ...
        for anchor in list(drawing_root):
            ...
            if referenced and not referenced.issubset(valid_rel_ids):
                drawing_root.remove(anchor)
        if not list(drawing_root):
            continue
        ...
        copied_count += len(list(drawing_root))
    ...
    try:
        ...
        try:
            ...
            if subject_code == "Q1":
                try:
                    q105_row_shifts = []
                    prior_q105_name = next(
                        (name for name in wb_prior_formula.sheetnames if name.startswith("Q1.05")),
                        None,
                    )
